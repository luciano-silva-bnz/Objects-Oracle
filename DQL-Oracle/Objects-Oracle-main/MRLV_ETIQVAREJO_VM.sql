CREATE OR REPLACE VIEW CONSINCO.MRLV_ETIQ_VAREJO_VM AS
...
--------- QUANDO ESTIVER NA OFERTA E QUANDO OFERTA FOI MENOR QUE PRECOO NORMAL
       ...
       CASE WHEN CONSINCO.NAGF_PRECOMN(A.CODACESSOPADRAO,A.NROEMPRESA) < NVL(H.PRECOVALIDPROMOC,0)-- SE EXISTIR MEU NAGUMO, FORMA OFERTA + MEU NAGUMO E FOR MENOR QUE OFERTA NORMAL
...
-- JOINS ALTERADOS POR GIULIANO - TABELAS INUTILIZADAS
-- ANTERIOR:
/*
FROM  CONSINCO.MRLX_BASEETIQUETAPROD A,
      --CONSINCO.MAP_PRODUTO B,
      --CONSINCO.MAP_FAMILIA C,
      --CONSINCO.MAP_MARCA D, -- NAO UTILIZA
...
FROM CONSINCO.MRLX_BASEETIQUETAPROD A INNER JOIN CONSINCO.MRL_PRODEMPSEG H            ON H.SEQPRODUTO = A.SEQPRODUTO AND H.NROEMPRESA = A.NROEMPRESA AND H.NROSEGMENTO = A.NROSEGMENTO AND H.QTDEMBALAGEM = A.QTDEMBCODACESSO
                                      INNER JOIN CONSINCO.MRLV_BASEETIQUETAPROD_NAG J ON J.NROEMPRESA = A.NROEMPRESA AND J.SEQPRODUTO = A.SEQPRODUTO AND J.NROSEGMENTO = A.NROSEGMENTO AND J.QTDEMBALAGEM = A.QTDEMBCODACESSO
