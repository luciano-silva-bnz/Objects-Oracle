create or replace view CONSINCO.MRLV_ETIQ_VAREJO_NG as
...
----------- Quando estiver na oferta e quando oferta foi MENOR que pre¿¿o normal 
       ...
       --chr(13) || chr(10) ||'^FO18,34^AQN,60,10^FD'|| substr(a.Desccompleta,0,40) || '^FS' -- Descri¿¿¿¿o Menor
         ...
         ||chr(13)|| chr(10)||'^FO18,07^AQN,20,20^FD' ||  'Val Prom.: ' || to_char(    -- Alterado por Giuiano - 03/08/2023 - Ticket 270721
         (SELECT MIN(XX.DTAINICIOPROM) FROM CONSINCO.MRL_PROMOCAOITEM XX WHERE XX.SEQPRODUTO = A.SEQPRODUTO AND XX.NROEMPRESA = A.NROEMPRESA AND SYSDATE BETWEEN XX.DTAINICIOPROM AND XX.DTAFIMPROM),'dd/mm/yy') || ' a ' ||to_char(
         (SELECT MAX(XX.DTAFIMPROM)    FROM CONSINCO.MRL_PROMOCAOITEM XX WHERE XX.SEQPRODUTO = A.SEQPRODUTO AND XX.NROEMPRESA = A.NROEMPRESA AND SYSDATE BETWEEN XX.DTAINICIOPROM AND XX.DTAFIMPROM),'dd/mm/yy')||'^FS'
...
from  consinco.mrlx_baseetiquetaprod a,
      ...
      consinco.mrlv_baseetiquetaprod j,
