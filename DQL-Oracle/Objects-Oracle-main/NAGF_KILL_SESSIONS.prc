...
BEGIN
  ...
  FROM GE_USUARIO
  WHERE CODUSUARIO = UPPER(p_usuario);-- AND NVL(NIVEL, 3) <= 3;
  ...
  IF usuario_encontrado > 0 THEN
    FOR t IN (SELECT SID, SERIAL#, INST_ID
             ...
             WHERE PROGRAM IN ('Recebimento.exe', 'Compras.exe')
               ...
               AND STATUS != 'ACTIVE'
               AND CLIENT_IDENTIFIER = UPPER(p_usuario))
    LOOP
      BEGIN
    ...
    RETURN 'Sessões do usuário '|| UPPER(p_usuario) ||' encerradas com sucesso.';
  ELSE
    RETURN 'Usuário '|| UPPER(p_usuario) ||' não encontrado! Verifique novamente.';
  END IF;
END;
