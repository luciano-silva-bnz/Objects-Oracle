CREATE OR REPLACE PROCEDURE CONSINCO.NAGP_ENVIA_INCONS_DDA_REDE AS

BEGIN

    FOR t IN (SELECT DISTINCT SEQREDE FROM CONSINCO.GE_REDEPESSOA GRD WHERE GRD.SEQPESSOA IN (
              SELECT DISTINCT F.SEQPESSOA FROM CONSINCO.FI_TITULO F INNER JOIN CONSINCO.FI_ESPECIE FI      ON F.CODESPECIE = FI.CODESPECIE AND F.NROEMPRESAMAE = FI.NROEMPRESAMAE
     ...
     BEGIN
       CONSINCO.NAGP_INCONS_DDA_REDE(T.SEQREDE);
