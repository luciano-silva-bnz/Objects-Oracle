CREATE OR REPLACE PROCEDURE CONSINCO.NAGP_GERA_ARQ_CD_FROMDEP (EMPRESA NUMBER) AS
    ...
    CURSOR CUR_LINHA IS
        SELECT X.LOCAL AS LOCAL,
               ...
               0,
               X.NROEMPRESA
          FROM CONSINCO.NAGV_MOVTO_FROMDEP X
    ...
    FOR REG_LINHA IN CUR_LINHA LOOP
        ...
        IF V_ATUAL_LOCAL IS NULL OR V_ATUAL_LOCAL != REG_LINHA.LOCAL THEN
            ...
            SAIDA := UTL_FILE.FOPEN(
                '/u02/app_acfs/arquivos/ocorrencias/Inventario_CD/Negativos',
                EMPRESA||' DEP_' || EMPRESA || '_PARA_' || REG_LINHA.LOCAL || '.TXT',
                'W'
            );
