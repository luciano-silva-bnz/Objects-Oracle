CREATE OR REPLACE PROCEDURE CONSINCO.NAGP_INCONS_DDA_REDE (vSeqRede IN NUMBER) AS

-- Criado por Giuliano - Versao por Rede
...
BEGIN
   ...
   vtexto := '<HTML>
          <style="color:red;">Informativo Automatico:
          <p style="font-family: Roboto, sans-serif;">Segue abaixo Titulos nao Conciliados via DDA para analise:<BODY bgColor=#ffffff>
          <p style="font-family: Arial, Helvetica, Sans Serif; font-size: 12px; color:#8f4d4d;">
          **Este e um projeto em desenvolvimento, pode ser necessario correcoes/ajustes**<BODY bgColor=#ffffff>
          <p style="font-family: Arial, Helvetica, Sans Serif; font-size: 12px; color:#8f4d4d;">
          **Os dados no campo *Inconsistencia* sao referentes aos arquivos DDA enviados pelo fornecedor, que *PODEM* ou nao estar relacionados aos titulos lancados**<BODY bgColor=#ffffff>
    ...
    <th width="20%" bgColor=#014ba0 >
      <B><FONT face=Calibri color=white size=2>Inconsistencia</FONT></B>
    </th>

    <th width="7%" bgColor=#014ba0 >
      <B><FONT face=Calibri color=white size=2>Titulo</FONT></B>
    </th>
    ...
    <th width="7%" bgColor=#014ba0 >
      <B><FONT face=Calibri color=white size=2>Vencimento C5</FONT></B>
         ...
         SELECT DESCRICAO 
           INTO vRede
           FROM CONSINCO.GE_REDE WHERE SEQREDE = vSeqRede;
...
CASE WHEN DIVERGENCIA IS NULL AND DOCTO IS NULL     THEN 'Titulo nao encontrado' 
WHEN DIVERGENCIA IS NULL AND DOCTO IS NOT NULL THEN 'Titulo encontrado - '||CASE WHEN LPAD(X.NROCNPJCPF, 12, 0) = (SELECT MAX(LPAD(D.NROCGCCPF, 12, 0)) FROM CONSINCO.GE_PESSOA D WHERE D.NROCGCCPF = X.NROCNPJCPF AND D.STATUS = 'A')
OR  LPAD(X.NROCNPJCPF,12,0)||LPAD(X.DIGCNPJCPF,2,0) IN
(SELECT LPAD(FR.NROCNPJCPF,12,0)||LPAD(FR.DIGCNPJCPF,2,0) FROM CONSINCO.FI_DDAREGRADETALHE FR WHERE FR.NROCNPJCPF = X.NROCNPJCPF)     
THEN 'Divergencia nao Identificada' ELSE 'CNPJ nao Cadastrado!' END
...
THEN NULL ELSE ' - CNPJ nao Cadastrado!' END END DIVERGENCIA, 
...
THEN 'Titulos Agrupados - DOC DDA: '||DDA2.NRODOCUMENTO||' - Valor Total: '||TO_CHAR(DDA2.VALORCOMDESCONTO,'FM999G999G999D90', 'nls_numeric_characters='',.''')
...
AND  (SELECT GEE.NROEMPRESA FROM CONSINCO.GE_EMPRESA GEE
WHERE LPAD(GEE.NROCGC,12,0)||LPAD(GEE.DIGCGC,2,0) = LPAD(A.NROCNPJCPFSACADO,12,0)||LPAD(A.DIGCNPJCPFSACADO,2,0)) IN(F.NROEMPRESA))
THEN 'Titulos Agrupados - Valor Sistema: '||TO_CHAR((
SELECT SUM(F2.VLRORIGINAL) - (SUM(NVL(FF3.VLRDESCCONTRATO,0)) + SUM(NVL(F2.VLRPAGO,0)))
AS VLRSOMADO
FROM FI_TITULO F2 INNER JOIN FI_COMPLTITULO FF3 ON F2.SEQTITULO = FF3.SEQTITULO
WHERE F2.SEQPESSOA = F.SEQPESSOA
...
INNER JOIN CONSINCO.FI_COMPLTITULO  FC ON F.SEQTITULO  = FC.SEQTITULO
  ...
  INNER JOIN CONSINCO.FIV_DDATITULOSBUSCA DDA2 ON /* Variaveis Valodarores nao Conciliados */
...
AND NVL(DDA2.ACEITO,'N') = 'N'
AND FC.CODBARRA IS NULL
AND F.SEQPESSOA IN (SELECT SEQPESSOA FROM CONSINCO.GE_REDEPESSOA GR WHERE GR.SEQREDE = vSeqRede)
...
WHERE XX.DIVERGENCIA NOT LIKE '%Divergencia nao Identificada%' 
  AND (XX.DIVERGENCIA != 'Titulo nao encontrado'
   OR XX.DIVERGENCIA NOT LIKE '%Titulo nao encontrado%' 
   OR XX.CNPJ_CADASTRADO LIKE '%N%'         AND XX.DIVERGENCIA != 'Titulo nao encontrado' 
   OR XX.CNPJ_CADASTRADO LIKE '%Bloqueado%' AND XX.DIVERGENCIA != 'Titulo nao encontrado' )
...
LOOP
                  ...
                  <tr>
                    ...
                    <tr> </tr>
                      ...
                      <p></p>
                        ...
                        E-mail Automatico - nao Responda
                      </p>
                    ...
                    <p style="font-family: Arial, Helvetica, Sans Serif; font-size: 12px; color:lightgray;">
                      Esta mensagem e seu conteudo tem carater absolutamente privativo e confidencial entre o remetente e o real destinatario, protegida pelas legislacoes brasileira e internacional. Se voce recebeu indevida ou equivocadamente esta mensagem, pedimos desculpas pelo inconveniente e solicitamos que seja deletado imediatamente a mensagem e seus anexos da sua caixa postal, bem como da sua lixeira, construindo potencial infracao o armazenamento indevido de qualquer das informacoes aqui veiculadas.
                    </p>
                    </font>
    ...
    --'lista.contasapagar@nagumo.com.br;giuliano.gomes@nagumo.com.br',--;
    'simone.eguti@nagumo.com.br;giuliano.gomes@nagumo.com.br',--marcel.cipolla@nagumo.com.br',
    'Inconsistencias DDA - Rede.: '||vRede|| ' - '||TO_CHAR(SYSDATE, 'DD/MM/YYYY'),
    vtexto,
    'S');
