CREATE OR REPLACE VIEW CONSINCO.NAGV_SIMULACAO_ETQ AS
SELECT DISTINCT /*+OPTIMIZER_FEATURES_ENABLE('11.2.0.4')*/
       A.SEQPRODUTO, A.NROEMPRESA, 'https://labelary.com/viewer.html' URL,
      '^XA' || '^PRA^FS' || '^LH00,00^FS'|| '^BY2^FS' || '^PQ' || NVL(A.QTDETIQUETA, 1) || '^FS'
       || CASE

--------- QUANDO ESTIVER NA OFERTA E QUANDO OFERTA FOR MENOR QUE PRECOO NORMAL
             WHEN (NVL(J.PRECOVALIDPROMOC,0) > 0) AND (NVL(J.PRECOVALIDPROMOC,0))  < TRUNC((S.PRECOVALIDNORMAL * 1),2)
               THEN
       -- ALTERADO POR GIULIANO | TRATAMENTO PRECO MEU NAGUMO
       CASE WHEN RPM.PRECOPPROMOCIONAL < NVL(J.PRECOVALIDPROMOC,0)-- SE EXISTIR MEU NAGUMO, FORMA OFERTA + MEU NAGUMO E FOR MENOR QUE OFERTA NORMAL

       THEN                                   CHR(13) || CHR(10) ||
        '^FO90,115^GB150,4,4^FS'           || CHR(13) || CHR(10) || -- RISCO
        '^FO050,190^BY2.4^BEN,25,Y,N^FD'||SUBSTR(fBusca_ean_max(A.SEQPRODUTO),0,12)||'^FS' || CHR(13) || CHR(10) || -- EAN
       ...
       ELSE -- OFERTA NORMAL
                                             CHR(13) || CHR(10) ||
        '^FO90,115^GB150,4,4^FS'          || CHR(13) || CHR(10) ||-- RISCO
        '^FO050,170^BY2.4^BEN,25,Y,N^FD'||SUBSTR(fBusca_ean_max(A.SEQPRODUTO),0,12)||'^FS'|| CHR(13) || CHR(10) ||
...
--======== INICIO OFERTAS ATIVAVEIS (MEU NAGUMO) - 27/08/2024 - Giuliano ========--
        ...
        WHEN AT.PRECOPROMOCIONAL < TRUNC((S.PRECOVALIDNORMAL * 1),2) AND A.NROEMPRESA NOT IN (31,41,54)
      THEN                                   CHR(13) || CHR(10) ||
        ...
        '^FO690,85^A0N,090,40^FD  ^FS'|| CHR(13) || CHR(10) ||
        '^FO050,180^BY2.4^BEN,25,Y,N^FD'||SUBSTR(fBusca_ean_max(A.SEQPRODUTO),0,12)||'^FS'|| CHR(13) || CHR(10) ||
        '^FO50,95^A0N,25,25^FDR$^FS'|| CHR(13) || CHR(10) ||
        ...
        '^FO80,140^A0N,25,25^FDPreco Normal^FS'|| CHR(13) || CHR(10) ||
        '^FO260,60^GB230,180,1^FS'|| CHR(13) || CHR(10) ||
        '^FO490,60^GB300,180,100^FR^FS'

--======== INICIO OFERTAS EXCLUSIVAS - APENAS MEU NAGUMO SEM OFERTA C5 - 02/09/2024 - Giuliano ========--

        WHEN NVL(J.PRECOVALIDPROMOC,0) = 0 AND RPM.PRECOPPROMOCIONAL < TRUNC((S.PRECOVALIDNORMAL * 1),2) AND A.NROEMPRESA NOT IN (31,41,54)
      THEN                                   CHR(13) || CHR(10) ||
        '^FO50,20^A0N,40,40^FD'||SUBSTR(A.DESCCOMPLETA,0,37) ||' '||CASE WHEN J.QTDEMBALAGEM > 1 THEN J.EMBALAGEM ELSE NULL END||'^FS' || CHR(13) || CHR(10) || -- DESC
        '^FO50,251^A0N,20,20^FD'||TO_CHAR(SYSDATE, 'DD/MM/YY HH24:MI')||'   Produto: '||A.SEQPRODUTO||'   Val. Prom.: '||
                                  TO_CHAR(RPM.DTINICIO, 'DD/MM/YY')||' a ' ||
                                  TO_CHAR(RPM.DTFIM,    'DD/MM/YY')|| '   LJ: ' || G.NOMEREDUZIDO||'^FS' || CHR(13) || CHR(10) ||
        '^FO274,75^A0N,20,20^FD- PRECO EXCLUSIVO -^FS'|| CHR(13) || CHR(10) ||
        '^FO323,175^A0N,20,20^FDApenas para ^FS'|| CHR(13) || CHR(10) ||
        '^FO270,210^A0N,20,20^FDCLIENTES IDENTIFICADOS^FS'|| CHR(13) || CHR(10) ||
        '^FO490,100^A0N,30,30^FD  R$  ^FS'|| CHR(13) || CHR(10) ||
        '^FO495,76^A0N,20,20^FD  POR  ^FS'|| CHR(13) || CHR(10) ||
        -- TRATATIVA LENGTH - TAMANHO PRECO PROMOC ATIVAVEL
        (CASE WHEN J.PRECOVALIDPROMOC > 99.99
        ...
        '^FO50,95^A0N,25,25^FDR$^FS'|| CHR(13) || CHR(10) ||
        '^FO51,125^A0N,20,20^FD^FS'|| CHR(13) || CHR(10) ||
        '^FO270,60,0^'||(SELECT LT_IMG FROM CONSINCO.NAGT_LT_IMG3 WHERE TIPO = 4) || CHR(13) || CHR(10) || -- Imagem MN
        -- TRATATIVA LENGTH - TAMANHO PRECO NORMAL
        (CASE WHEN J.PRECOVALIDNORMAL > 99.99
        THEN
        '^FO40,75^A0N,62,62^FD'||LPAD(TRUNC(J.PRECOVALIDNORMAL), 4, ' ' ) || ',' || LPAD((J.PRECOVALIDNORMAL - TRUNC(J.PRECOVALIDNORMAL)) * 100, 2, '0')|| '^FS'
...
----------- INICIO PRECO UNICO NORMAL OU PROMOCAO FOR MAIOR QUE NORMAL
         WHEN /*ROUND (NVL(S.PRECOVALIDPROMOC,S.PRECOVALIDNORMAL)/1,2) = TRUNC((S.PRECOVALIDNORMAL/1),2) OR*/
           NVL(S.PRECOVALIDPROMOC,0) >= S.PRECOVALIDNORMAL OR  NVL(S.PRECOVALIDPROMOC,0) = 0 AND S.PRECOVALIDNORMAL > 0
           THEN  CHR(13) || CHR(10) ||
        '^FO50,20^A0N,40,40^FD'||SUBSTR(A.DESCCOMPLETA,0,37) ||' '||CASE WHEN J.QTDEMBALAGEM > 1 THEN J.EMBALAGEM ELSE NULL END||'^FS' || CHR(13) || CHR(10) || -- DESC
        '^FO050,200^BY2.4^BEN,30,Y,N^FD' ||SUBSTR(fBusca_ean_max(A.SEQPRODUTO),0,12)||'^FS' --EAN COD BARRAS
      --- QR CODE ETIQUERA NORMAL
      || CHR(13) || CHR(10) || '^FO050,075^BQ,2,4^FDLA,:p:' || '1:vp:1' || '^FS'
      || CHR(13) || CHR(10) ||'^FO180,85^A0N,110,60^FDR$^FS' ||'^FS' -- R$
      || CHR(13) || CHR(10) ||'^FO350,90^A0N,170,130^FD'|| LPAD(TRUNC(S.PRECOVALIDNORMAL), 4, ' ' ) || ',' || --PRECO NORMAL
                                                           LPAD((S.PRECOVALIDNORMAL - TRUNC(S.PRECOVALIDNORMAL)) * 100, 2, '0')|| '^FS'

----PRECO POR KG OU LI OU ETC...
||    (CASE WHEN
       J.MULTEQPEMB IS NOT NULL  OR J.MULTEQPEMBALAGEM IS NOT NULL
       THEN
CHR(13) || CHR(10) || '^FO480,255^A0N,20,19^FD' ||/* '*PRECO PAGO POR ' || J.MULTEQPEMBALAGEM ||' R$   '*/'Nesta Embalagem ' || CASE WHEN J.MULTEQPEMBALAGEM = 'LI' THEN 'L'
...
END)||'^FR^FS'
 ...
 ------------INICIO PRECO UNICO CASO NAO ESTEJA EM OFERTA E TENHA PRECOS DIF POR EMBALAGENS
         WHEN TRUNC(NVL(S.PRECOVALIDPROMOC,S.PRECOVALIDNORMAL)/1,2) = TRUNC((S.PRECOVALIDNORMAL/1),2) AND (NVL(S.PRECOVALIDPROMOC,0) = 0)
           THEN  CHR(13) || CHR(10) ||
      '^FO50,20^A0N,40,40^FD'||SUBSTR(A.DESCCOMPLETA,0,37) ||' '||CASE WHEN J.QTDEMBALAGEM > 1 THEN J.EMBALAGEM ELSE NULL END||'^FS' || CHR(13) || CHR(10) || -- DESC
    --  || CHR(13) || CHR(10) || '^FO050,200^BY2.4^BEN,30,Y,N^FD' ||SUBSTR(fBusca_ean_max(A.SEQPRODUTO),0,12)||'^FS' --EAN COD BARRAS
       DECODE(LENGTH(SUBSTR(fBusca_ean_max(A.SEQPRODUTO),0,12)),13, '^FO050,200^BY2.4^BEN,30,Y,N^FD', '^FO050,200^BY2.4^BCN,30,Y,N^FD' ) ||SUBSTR(fBusca_ean_max(A.SEQPRODUTO),0,12)||'^FS' --EAN COD BARRAS BARRAS
        --- QR CODE ETIQUERA NORMAL
      || CHR(13) || CHR(10) ||'^FO050,075^BQ,2,4^FDLA,:p:' || '1:vp:1' ||'^FS'
      || CHR(13) || CHR(10) ||'^FO180,85^A0N,110,60^FDR$^FS' ||'^FS' -- R$
      || CHR(13) || CHR(10) ||'^FO350,90^A0N,170,130^FD'|| LPAD(TRUNC(S.PRECOVALIDNORMAL), 4, ' ' ) || ',' || --PRECO NORMAL
...
--FIM DA ETIQUETA
|| CHR(13) || CHR(10) || '^XZ'
|| CHR(13) || CHR(10) LINHA_ZPL

FROM CONSINCO.MRLV_BASEETIQUETAPROD_NAGV2 A  --LEFT JOIN CONSINCO.MAP_PRODCODIGO C ON C.SEQPRODUTO = A.SEQPRODUTO AND C.TIPCODIGO = 'E' AND C.QTDEMBALAGEM = A.qtdembalagem
                                             LEFT JOIN CONSINCO.MRLV_BASEETIQUETAPROD_NAGV2 J ON J.NROEMPRESA  = A.NROEMPRESA AND J.SEQPRODUTO = A.SEQPRODUTO AND J.NROSEGMENTO = A.NROSEGMENTO AND J.QTDEMBALAGEM = A.qtdembalagem
                                             LEFT JOIN CONSINCO.MAX_EMPRESA G               ON G.NROEMPRESA  = A.NROEMPRESA
                                             INNER JOIN CONSINCO.MRL_PRODEMPSEG S ON S.NROEMPRESA = A.NROEMPRESA AND S.SEQPRODUTO = A.SEQPRODUTO AND A.nrosegmento = 2 AND S.QTDEMBALAGEM = A.qtdembalagem
                                       ...
                                       LEFT JOIN (SELECT PRECOPPROMOCIONAL, CODLOJA, CODIGOPRODUTO, RP.DTINICIO, RP.DTFIM
                                                   ...
                                                   WHERE SYSDATE BETWEEN RP.DTHRINICIO AND RP.DTHRFIM -- Ajustado a view para tratativa de hora/min na DTFIM | 19/03/2024
                                                     ...
                                                     AND RP.PROMOCAOLIVRE = 0) RPM ON RPM.CODLOJA = A.NROEMPRESA
                                                                                 AND RPM.CODIGOPRODUTO = LPAD(SUBSTR(fBusca_ean_max(A.SEQPRODUTO),0,12),14,0)
