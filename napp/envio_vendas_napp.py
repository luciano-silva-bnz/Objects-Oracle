...
def process_data_and_call_api():
    ...
    #catalogo_df['NCM'] = catalogo_df['NCM'].astype(str).str.zfill(8)
    ...
    dados_venda_df.to_csv(dados_venda_csv, index=False, sep=';', header=True)
    #itens_venda_df.to_csv(itens_venda_csv, index=False, sep=';', header=True)
    #catalogo_df.to_csv(catalogo_csv, index=False, sep=';', header=True)
    ...
    if response is None:
        print("Erro ao obter URLs assinadas. Processo abortado.")
        return
    
    upload_urls = response.get("uploadFiles", [])
    
    if not upload_urls:
        print("Nenhuma URL assinada retornada. Processo abortado.")
        return

    # Fazer upload dos arquivos para as URLs assinadas
    renamed_files = rename_local_files([file_info['name'] for file_info in upload_urls],
                                       [dados_venda_csv#, itens_venda_csv, catalogo_csv
                                        ]
                                        )

    upload_to_storage([file_info['url'] for file_info in upload_urls], renamed_files)
